...
def saveLogFile(page,postfix):
        with open(f'phaseswitch{postfix}.log', 'w') as writer:
            writer.write(page)
...
#        filtered = session.cookie_jar.filter_cookies("http://" + ip)
        ...
        async with session.post(
            ...
            token = parsedPage.select('input[name="token"]')[0]["value"]
            async with session.post(
...
if __name__ == "__main__":
    ...
    parser.add_argument('current', nargs='?', default=16)
